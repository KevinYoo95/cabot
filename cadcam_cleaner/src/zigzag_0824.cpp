...
double udir1_lower = 0.0;
double udir1_upper = 0.0;
double udir2_lower = 0.0;
double udir2_upper = 0.0;
double uturn_offset = 0.05;
...
void Pose_Estimate(){
    ...
    if(pose_flag == 0){
        ...
        if(start_flag == 1){
            ...
            if((front > start_ang - 0.1)&&(front < start_ang + 0.1)){
                wheelController(0,0);
                pose_flag = 1;
                start_flag = 0;
            }
        ...
        if(((left_side < 0.45)&&(left_30 < 0.6))&&(left_60 < 0.6)){
            wheelController(0.0,0.0);
            pose_flag = 3;
        }
        else{
        ...
        if(front < 0.4){
            ...
            std::cout<< "wall_follow" << std::endl;
            if(turn_flag == 0){
                dist_cur =  left_side + 0.5 * sin(3.141592*(degree_min-90.0)/180.0); 
                if((left_side > 0.4) && (left_upper > 0.7)){
                    //turn_flag = 1;
                    if(stop_flag>2){
                        turn_flag = 1;
                    }
...
void scanCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    for(int i = 0; i < count; i++){
            ...
            if(degree > -180 && degree< 180){
                if((scan->ranges[i] >= 0.05) && (scan->ranges[i]<1.0)){
...
{   
   ...
   while (ros::ok())//ros::ok())
   {
    ...
    if((left_side<0.27)||(right_side<0.27)){
            tactile_flag = 0;
        }
        else{
            tactile_flag = 1;
    ...
    if(big_flag==0){
        if(((Long < 10.0) && (Short < 10.0)) && (yawCov< 15.0)){
            big_flag=2;
        }
        else{
            //client.call(srv);
        ...
        if(((Long < 10.0) && (Short < 10.0)) && (yawCov< 15.0)){
            ...
            if(diRection1 > (3.1415-uturn_offset)){
                udir1_upper = diRection1 + uturn_offset - 3.1415*2;
                udir1_lower = diRection1 - uturn_offset;
            }
            else if(diRection1 < (-3.1415+uturn_offset)){
                udir1_upper = diRection1 + uturn_offset;
                udir1_lower = diRection1 - uturn_offset + 3.1415*2;
            }
            else{
                udir1_upper = diRection1 + uturn_offset;
                udir1_lower = diRection1 - uturn_offset;
            ...
            if(diRection2 > (3.1415-uturn_offset)){
                udir2_upper = diRection2 + uturn_offset - 3.1415*2;
                udir2_lower = diRection2 - uturn_offset;
            }
            else if(diRection2 < (-3.1415+uturn_offset)){
                udir2_upper = diRection2 + uturn_offset;
                udir2_lower = diRection2 - uturn_offset + 3.1415*2;
            }
            else{
                udir2_upper = diRection2 + uturn_offset;
                udir2_lower = diRection2 - uturn_offset;
        ...
        std::cout<< "dir1_upper : "<< udir1_upper << "dir1_lower : "<< udir1_lower<<std::endl;
        std::cout<< "dir1 : "<< diRection1 << " dir2 : "<< diRection2 <<std::endl;
        std::cout<< "cur_yaw : "<< yaw << " t_flag : "<< t_flag <<std::endl;

        if(t_flag == 0){ /// upward
            if((((((left_30 < 0.33)||(right_30 < 0.33))&&(tactile_flag == 1)) || (front < 0.45)))&&(u_flag == 0)){
                u_flag = 1;
                right_sp = 0.0;
                left_sp = 0.1;
            ...
            if(udir1_upper*udir1_lower <= 0){
                if((yaw > udir1_upper) || (yaw < udir1_lower)){
            ...
            if((((((left_30 < 0.32)||(right_30 < 0.32))&&(tactile_flag == 1))||(front < 0.45)))&&(u_flag == 0)){
                u_flag = 1;
                right_sp = 0.1;
                left_sp = 0.0;
            ...
            if(udir2_upper*udir2_lower <= 0){
                if((yaw > udir2_upper) || (yaw < udir2_lower)){
                    t_flag = 0;
                    u_flag = 0;
                }
            }
