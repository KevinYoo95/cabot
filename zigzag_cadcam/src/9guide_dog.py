...
class ZigZag_Ver5():
	def __init__(self):
		...
		self.dis_criterion = 0.4
		self.dis_ = 0.2

		self.ith = 0
		self.jumhyul_dis = 1 #pixel
		self.lin_gap = 5 # pixel
		...
		rospy.init_node("Guide_dog_node")
	...
	def distance_straight(self):
		...
		#print(self.ith,self.ith_publish)
		for i in range(len(self.line_coor)-self.ith):
			...
			if( (  ((self.line_coor[self.ith][0] - robot_x)*(self.line_coor[self.ith][0] - robot_x) + (self.line_coor[self.ith][1]-robot_y)*(self.line_coor[self.ith][1]-robot_y)) >(self.dis_criterion*self.dis_criterion) ) or (self.line_coor[self.ith + self.jumhyul_dis][2]!=0)):
				...
				#print("self.ith" , self.ith , (  ((self.line_coor[self.ith][0] - robot_x)*(self.line_coor[self.ith][0] - robot_x) + (self.line_coor[self.ith][1]-robot_y)*(self.line_coor[self.ith][1]-robot_y))  ),self.dis_criterion*self.dis_criterion)
				#print(robot_x,self.line_coor[self.ith][0],robot_y,self.line_coor[self.ith][1])
	...
	def update(self):
		if(self.map_flag ==1):
			...
			#print(self.robot_pose.pose.position.x,self.line_coor[self.ith][0],self.robot_pose.pose.position.y,self.line_coor[self.ith][1])
			...
			if((self.interpolator_flag ==1) ):	## Starting:True False -> on line: False False ->reaching goal point True True
				...
				if(self.flag2 == False):
					...
					if(self.flag1 == False):    ##publish once
						self.zig()
						self.counting_flag = False
						self.flag1 = True
