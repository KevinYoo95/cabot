...
class ZigZag_Ver5():
			...
			if(i<incre-1):
				if(self.up_down_flag%2==0):
					...
					for j in range(y-now[1]):
						self.line_only.append([now[0],now[1]+j])
						print(now,[now[0],now[1]+j],[x,y],"0")
				else:
	...
	def cubicspline(self):
		x = []
		y = []
		for i in range(len(self.line_coor)):
			x.append(self.line_coor[i][0])
			y.append(self.line_coor[i][1])

		cs = CubicSpline(x, y)
		for i in range(len(self.line_coor)):
			self.splined_line.append([x[i],y[i]])
	...
	def publish_waypoints(self):
   	...
   	def making_line_map(self):
		...
		for i in range(len(self.line_only)):	
			...
			#print(len(self.trajectorymap.data),num)
	...
	def update(self):
		if(self.map_flag ==1):
			if(self.drawing_line_flag ==0):
				...
				self.pixel_to_meter()
				self.cubicspline()
				self.line_only = self.splined_line
