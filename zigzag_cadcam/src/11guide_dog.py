...
class ZigZag_Ver5():
	def __init__(self):
		...
		self.pix2 = []
		self.pix3 = []
		self.infl_factor = 3+3 # pixel -> 15cm + 10cm = 25cm of inflation
		self.base_x =0
		self.base_y = 0
	...
	def draw_(self):
		...
		for i in range(incre):	
			...
			if(i!=0):
				self.line_only[n-nn-1][2] = 3
			nn = len(self.line_only)
	...
	def pixel_to_meter(self):
   	...
   	def making_line_map(self):
		...
		#print(len(self.inf_space)
		#self.trajectorymap_data = self.inf_space
	...
	def distance_straight(self):
		for i in range(len(self.line_coor)-self.ith):
			...
			if(((self.line_coor[self.ith][0] - self.robot_x)*(self.line_coor[self.ith][0] - self.robot_x) + (self.line_coor[self.ith][1]-self.robot_y)*(self.line_coor[self.ith][1]-self.robot_y) >self.dis*self.dis) or (self.line_coor[self.ith + self.jumhyul_dis][2]!=0)):
				break
	...
	def distance_curved(self):
		for i in range(len(self.line_coor)-self.ith):
			...
			self.estimate_current_position()
			if(((self.line_coor[self.ith][0] - self.robot_x)*(self.line_coor[self.ith][0] - self.robot_x) + (self.line_coor[self.ith][1]-self.robot_y)*(self.line_coor[self.ith][1]-self.robot_y) >self.dis*self.dis)or (self.line_coor[self.ith + self.jumhyul_dis][2]!=2)):
				break
			self.ith= self.ith+self.jumhyul_dis*i
	...
	def distance_turning(self):
		for i in range(len(self.line_coor)-self.ith):
			...
			self.estimate_current_position()
			if(((self.line_coor[self.ith][0] - self.robot_x)*(self.line_coor[self.ith][0] - self.robot_x) + (self.line_coor[self.ith][1]-self.robot_y)*(self.line_coor[self.ith][1]-self.robot_y) >self.dis*self.dis) or (self.line_coor[self.ith + self.jumhyul_dis][2]!=3)):
				break
			self.ith= self.ith+self.jumhyul_dis*i
	...
	def estimate_current_position(self):
		quaternion_amcl =[self.amcl_pose.pose.orientation.x,self.amcl_pose.pose.orientation.y,self.amcl_pose.pose.orientation.z,self.amcl_pose.pose.orientation.w]
		euler_amcl = euler_from_quaternion(quaternion_amcl)
		quaternion_odom =[self.robot_pose.pose.orientation.x,self.robot_pose.pose.orientation.y,self.robot_pose.pose.orientation.z,self.robot_pose.pose.orientation.w]
		euler_odom = euler_from_quaternion(quaternion_odom)

		self.robot_x = self.amcl_pose.pose.position.x + (self.robot_pose.pose.position.x)*(math.cos(euler_amcl[2])) - (self.robot_pose.pose.position.y)*(math.sin(euler_amcl[2]))
		self.robot_y = self.amcl_pose.pose.position.y + (self.robot_pose.pose.position.x)*(math.sin(euler_amcl[2])) + (self.robot_pose.pose.position.y)*(math.cos(euler_amcl[2]))
	...
	def update(self):
		if(self.map_flag ==1):
			...
			if((self.interpolator_flag ==1) ):	## Starting:True False -> on line: False False ->reaching goal point True True
				...
				print(self.robot_x,self.robot_y,self.line_coor[self.ith])
				if(self.flag2 == False):
					self.zig()
