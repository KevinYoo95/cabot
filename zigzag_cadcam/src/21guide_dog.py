...
class Guide_Dog():
	...
	def c_cell_cal(self):
		...
		#cv2.drawContours(dilation , contours[0] , -1 , (255,255,255) , 1)

		cv2.imshow('ddata2', ddata2)
	   	cv2.waitKey(2000)
	...
	def find_left_up_point(self):
   	...
   	def mapp_line_point(self):
		...
		self.trajectorymap.info.origin.position.x = -1
		self.trajectorymap.info.origin.position.y = -1
   	...
   	def making_line_map(self):
		...
		#for i in range(3):
		#	self.trajectorymap.data[1 + self.height*i]=100
		#	self.trajectorymap.data[0 - self.height*i]=100
		#for i in range(self.height*self.width):
		#	self.trajectorymap.data.append(0)
	...
	def drawing_path(self):
		...
		for i in range(incre):
			self.draw_lines(r_c_point + self.width * self.lin_gap*(i))
				...
				if(self.up_down_flag%2==0):
					...
					for j in range(self.lin_gap):
						if(now[0]+(now[1]+j)*self.width<self.height*self.width):
							if(self.map_array[now[0]+(now[1]+j)*self.width] != 100):
								self.line_only.append([now[0],now[1]+j,3])
				else:
					y,x = divmod(now[0] + (now[1]+ self.lin_gap)*self.width , self.width)
	
					for j in range(self.lin_gap):
						if(now[0]+(now[1]+j)*self.width-1<self.height*self.width):
							if(self.map_array[now[0]+(now[1]+j)*self.width-1] != 100):
								self.line_only.append([now[0],now[1]+j,3])
	...
	def draw_lines(self, starting_array): 
		...
		if(self.up_down_flag%2==0):
			for i in range(self.width-xx):
				x = i
				y = yy
				j = x + y*self.width
				flag = 0
				if(j < self.height*self.width):
					#print(x,y,j,self.map_array[j],len(self.line_only))
	...
	def pixel_to_meter(self):
		for j in range(int(len(self.line_only))):
			...
			s_y = (self.line_only[j][1]+(self.originx/self.resolution))*self.resolution
