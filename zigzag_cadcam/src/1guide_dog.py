...
class ZigZag_Ver5():
	...
	def draw_lines(self,starting_array): 
		...
		if(self.up_down_flag%2==0):
			for i in range(int((self.height-xx)/self.chomchom)):
				x= xx+i*self.chomchom
				y = yy
				j = x + y*self.height
				flag = 0
	...
	def planning(self):	
		...
		#print(self.line_num1)
        	self.goal_pose[0] = self.line_coor[self.line_num1][0]
        	self.goal_pose[1] = self.line_coor[self.line_num1][1]
        	self.goal_pose[2] = 0
		if(self.line_num1<(len(self.line_coor)-1)):
			self.determine_next_orientation(self.line_num1)
	...
	def publish_waypoints(self):
		...
		if(self.line_only is not []):
			for w in self.line_coor:				
				...
				if(line_num<(len(self.line_coor)-2)):
